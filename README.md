...
- **Codebase and web understanding.** Pierre Dreb includes file, grep/find/ls, bash, web search/fetch, task tracking, skill invocation, semantic `search`, and bounded `repo_graph` traversal. In a Git repository, each top-level Pierre Dreb startup automatically builds or incrementally refreshes the local structural graph without generating embeddings; semantic search adds vectors on demand. Semantic search uses AST-aware chunks, embeddings, POEM ranking, memory indexing, and a shared static file-import graph; it also ships as [`@dreb/semantic-search`](packages/semantic-search/) with an MCP server for other harnesses. The semantic search package requires Node.js 22+.
- **Detailed usage tracking and performance logging.** Pierre Dreb records per-session token usage, cost, context-window utilization, and rolling tokens-per-second performance in a local JSONL log (`~/.dreb/agent/performance.jsonl`). Session JSONL trees also provide same-local-day cost accounting across main sessions and all descendant subagents. The TUI footer separates the current session's `main` and `children` costs from the all-session `today` total, shows a hierarchical per-child breakdown, and emits warning-only notices at $50, $100, and every additional $50 without stopping work. This data stays on your machine; dashboard session details, Telegram `/stats`, and RPC share the same latest-100 median with long-term delta for personal analytics and model comparison.
- **Safety and reliability primitives.** Recent dreb-specific hardening includes secret output scrubbing, sensitive-file guards, destructive-command guards, resource diagnostics surfaced in-session, warning propagation, rate-limited web search across parallel subagents, and JSON/RPC protocol hardening. Dropped provider streams are retried (discarding the partial), and responses truncated at the model's output-token limit are retried with a larger token budget. Genuine output-budget exhaustion fails loudly; truncation caused by a full context window enters compact-and-retry recovery.
- **Multiple interfaces.** Run Pierre Dreb as an interactive TUI, print/headless CLI, JSON event stream, RPC process, embedded [SDK](packages/coding-agent/docs/sdk.md), [web dashboard](packages/coding-agent/docs/dashboard.md), or [Telegram bot](packages/telegram/).

## Quick Start

> **Node.js 22 LTS is required.** Pierre Dreb relies on SSE streaming behavior that is stable in Node 22 LTS. Node 24 and Node 26 are known to break provider streaming due to changes in ReadableStream buffering, which causes every provider to fail with **"request ended without sending any chunks"**. If you see that error, switch to Node 22 LTS.

### Building from source (recommended)

```bash
git clone https://github.com/chemdalf-work/pierre-dreb.git
cd pierre-dreb
npm install
npm run build
...
The retained package namespace is `@dreb/*`; installation exposes canonical `pierre-dreb` and compatibility `pierre-dreb` commands that point to the same compiled CLI.
...
Inspect the exact build and runtime identity without loading credentials or extensions:

```bash
pierre-dreb diagnostics
pierre-dreb diagnostics --json
```
...
**Bun users:** Bun's lockfile can cache stale versions of `@dreb/*` packages, causing import errors after upgrades. If you hit missing export errors with `bunx pierre-dreb`, clear the cache and re-install:

```bash
bun pm cache rm
bunx --force pierre-dreb
```

### Troubleshooting

- **"request ended without sending any chunks" on every provider** — Your Node version is likely too new. Switch to **Node.js 22 LTS**. Node 26 in particular changed ReadableStream buffering in a way that breaks the Anthropic and OpenAI SDK stream parsers Pierre Dreb uses.

## Core capabilities

### Tools and interaction

Pierre Dreb ships with 13 standard built-in tools enabled by default: `read`, `write`, `edit`, `bash`, `grep`, `find`, `ls`, `web_search`, `web_fetch`, `subagent`, `wait`, `watch_github_ci` (blocks until pull-request checks pass or fail), and `ask_user` (pause and ask the user one or more structured multiple-choice or free-text clarifying questions in a single batch wizard, with Markdown-formatted question text and an in-card Stop agent action, rendered natively in the TUI and Dashboard). Setting `backgroundAgents.maxConcurrentSubagents` to `0` removes `subagent` from new parent sessions. Four additional tools are always active: `search` for semantic codebase search, `repo_graph` for bounded static dependency traversal, `skill` for loading workflows, and `tasks_update` for visible task tracking. `suggest_next` (ghost text command suggestions, Tab to accept) is active by default but excluded when `--tools` is specified.
...
Pierre Dreb packages make those resources installable and shareable through npm, git, URLs, or local paths. Use `pierre-dreb install`, `pierre-dreb list`, `pierre-dreb update`, and `pierre-dreb config` to manage them; project-local packages can be checked into settings so teams get the same skills, extensions, prompts, and themes.

### Optional context-efficient analysis

The separately maintained [`dreb-context-mode`](https://github.com/chemdalf-work/dreb-context-mode) package adds advisory, context-efficient analysis routing without adding a generic MCP client to Pierre Dreb's core. Install it once:

```bash
pierre-dreb install git:github.com/chemdalf-work/dreb-context-mode
```
...
This is external package code, not an OS sandbox: its child process has the Pierre Dreb user's filesystem, network, executable, and other OS privileges. Operations that index data persist it only in the package's dreb-owned local storage, with project isolation, retention, removal, and abandoned-directory pruning documented by the [package](https://github.com/chemdalf-work/dreb-context-mode); Pierre Dreb core remains without a generic MCP client. Review the package before installing it. RTK is intentionally not integrated because fidelity, exit-code, and actionable-diagnostic failures made automatic interception unsafe.
...
- **Print/headless CLI** — `pierre-dreb -p` for one-shot prompts, including piped stdin.
...
- **Long-horizon supervisor** — `@dreb/long-horizon` durably drives bounded SDK sessions across planning, execution, rollover, recovery, and acceptance.
- **Telegram** — `@dreb/telegram` runs Pierre Dreb as a bot with sessions, model controls, file upload/download, live tool status, and visible results for user-facing tools.
- **Web dashboard** — `pierre-dreb dashboard` serves a browser UI (fleet overview of all sessions, full chat with steering, subagent observability, host file browser, Pierre Dreb memory editor); local-only by default, remote via Tailscale + rotating pairing code. See [dashboard docs](packages/coding-agent/docs/dashboard.md).

### Web dashboard

The dashboard is the visual face of Pierre Dreb: every agent session on the host, live in the browser, with the same fidelity as the terminal.
...
**Automatic tab titles.** New unnamed sessions can be named automatically after a configurable number of tool calls. Dashboard Settings can enable or disable generation and pin its one-shot call to an exact `provider/model` or clear it back to the automatic route; when no dedicated model is set, Pierre Dreb preserves the Explore-agent route and parent-session fallback. See [Tab Title settings](packages/coding-agent/docs/settings.md#tab-title).

**Low-data transcript images.** Tool results and images uploaded with user turns remain visible through content-addressed browser references rather than full base64 in events and transcript JSON. In dashboard mode each unique image crosses the RPC stdout pipe at most once per child process — later occurrences are tiny references — and the child's stdout queue has a 64 MiB hard bound plus a 30 s no-drain watchdog, so multi-image bursts can drain without allowing unbounded growth or indefinite stalls. The default browser-local mode lazily requests a preview bounded to 1024 × 1024 and 256 KiB; clicking enlarges that same preview without downloading the original. Settings also offers request-free placeholders and informed-opt-in automatic originals. Explicit originals disclose their size and confirm above 1 MiB. Authenticated same-origin routes accept only signature-matching PNG/JPEG/GIF/WebP, send `nosniff`, and recover evicted entries from authoritative session data; static GIF previews preserve animated originals behind the original route. Image bytes therefore cannot cause an SSE oversized-event resync. Full-resolution HTML exports remain self-contained and unchanged.
...
Pierre Dreb is a hard fork of [pi-mono](https://github.com/badlogic/pi-mono), itself derived from Claude Code. Claude Code is a great product; Pierre Dreb is not trying to win by cloning every feature into a bigger built-in core. It is trying to win on control, hackability, provider choice, and inspectable workflows.
...
- **No mandatory permission-popup system.** Run in a container, rely on Pierre Dreb's guards, or build the confirmation flow you want with extensions.
...
A hard fork means Pierre Dreb controls the update cadence. Upstream changes do not land automatically; useful fixes can be cherry-picked, product direction can diverge, and dreb-specific work such as mach6, memory maintenance, Telegram, safety guards, and provider routing can evolve on its own schedule.
...
| [`@dreb/long-horizon`](packages/long-horizon/) | Durable SDK supervisor with role-based rounds, recovery, safe rollover, OS-sandboxed credential and remote-state boundaries, and acceptance gates |
| [`@dreb/telegram`](packages/telegram/) | Telegram bot frontend for Pierre Dreb over the native RPC protocol |

...
- **Durable context.** [Sessions](packages/coding-agent/docs/session.md) are JSONL trees with resume/continue, `/tree` navigation, `/fork`, CLI `--fork`, compaction, HTML export, and JSONL import/export. The standalone [`@dreb/long-horizon`](packages/long-horizon/) supervisor adds durable multi-session goal rounds, Sol/Terra role routing, failure escalation, safe context rollover, recovery, and evidence-gated completion. The narrower opt-in [auto-compaction continuation setting](packages/coding-agent/docs/compaction.md#continuing-after-auto-compaction) only starts another model turn after successful automatic compaction. [Memory](packages/coding-agent/README.md#memory) is file-based, global + project-scoped, survives sessions, can read Claude Code project memory, and can be maintained with `/dream` memory consolidation.

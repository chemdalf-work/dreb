...
TypeBox exports are re-exported from `@dreb/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@dreb/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@dreb/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@dreb/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@dreb/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@dreb/ai';
...
```typescript
import { getModel, stream } from '@dreb/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@dreb/ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@dreb/ai';
...
```typescript
import { getModel, complete, Context } from '@dreb/ai';
...
```typescript
import { Context, getModel, complete } from '@dreb/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@dreb/ai/oauth` entry point in Node.js.
...
Set `DREB_AI_ANTIGRAVITY_VERSION` to override the Antigravity User-Agent version when Google updates their requirements:

```bash
export DREB_AI_ANTIGRAVITY_VERSION="1.23.0"
```

#### Cache Retention

Set `DREB_CACHE_RETENTION=long` to extend prompt cache retention:

| Provider | Default | With `DREB_CACHE_RETENTION=long` |
|----------|---------|-------------------------------|
...
The library provides login and token refresh functions via the `@dreb/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
```typescript
import { loginGitHubCopilot } from '@dreb/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@dreb/ai`

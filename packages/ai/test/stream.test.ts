...
describe("Generate E2E Tests", () => {
	...
	describe.skipIf(process.env.DREB_SKIP_LIVE_API === "1" || !hasBedrockCredentials())(
		...
		() => {
			...
			it("should pass requestMetadata to the SDK payload", { retry: 3 }, async () => {
				...
				let capturedPayload: unknown;
				const metadata = { app: "dreb-test", env: "ci" };
				const response = await complete(
					llmSonnet,
					{
						messages: [
	...
	// `ollama list` fails if the server isn't reachable; grep fails if the model isn't available.
	// Both must succeed for tests to run — no silent skips, no auto-pull, no server lifecycle management.
	let ollamaReady = false;
	if (!process.env.DREB_NO_LOCAL_LLM) {
		try {
			execSync("ollama list | grep -q 'gpt-oss'", { stdio: "ignore" });
			ollamaReady = true;
		} catch {
			ollamaReady = false;
		}
	...
	describe.skipIf(!ollamaReady)("Ollama Provider (gpt-oss-20b via OpenAI Completions)", () => {
		const llm: Model<"openai-completions"> = {
			id: "gpt-oss:20b",
			api: "openai-completions",
			provider: "ollama",

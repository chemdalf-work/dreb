...
const PROVIDER_MODEL_PAIRS: ProviderModelPair[] = [
	...
	// OpenAI Codex
	{ provider: "openai-codex", model: "gpt-5.6-luna", label: "openai-codex-gpt-5.6-luna" },
	// Google Antigravity
	...
	// GitHub Copilot
	{ provider: "github-copilot", model: "claude-opus-4.7", label: "copilot-claude-opus-4.7" },
...
function dumpFailurePayload(params: { label: string; error: string; payload?: unknown; messages: Message[] }): void {
	const filename = `/tmp/dreb-handoff-${params.label}-${Date.now()}.json`;
	const body = {
		label: params.label,
		error: params.error,

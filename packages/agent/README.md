...
Stateful agent with tool execution and event streaming. Built on `@dreb/ai`.

## Installation

```bash
npm install @dreb/agent-core
```
...
```typescript
import { Agent, streamProxy } from "@dreb/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@dreb/agent-core";
